export type ConeTempEntry = {
  cone: string;
  temperatureF: number;
};

export const DEFAULT_CONE_CHART: ConeTempEntry[] = [
...
const STORAGE_KEY = "kiln-firing-temp-chart";

export const seedConeChart = () => {
  if (typeof window === "undefined") return DEFAULT_CONE_CHART;
  const existing = window.localStorage.getItem(STORAGE_KEY);
  if (!existing) {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(DEFAULT_CONE_CHART));
    return DEFAULT_CONE_CHART;
  }
  try {
    const parsed = JSON.parse(existing);
    if (Array.isArray(parsed) && parsed.every((entry) => typeof entry?.cone === "string")) {
      return parsed as ConeTempEntry[];
    }
  } catch (error) {
    console.error("Failed to parse stored cone chart", error);
  }
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify(DEFAULT_CONE_CHART));
  return DEFAULT_CONE_CHART;
};

export const loadConeChart = () => {
  if (typeof window === "undefined") return DEFAULT_CONE_CHART;
  try {
    const stored = window.localStorage.getItem(STORAGE_KEY);
    if (!stored) return DEFAULT_CONE_CHART;
    const parsed = JSON.parse(stored);
    if (Array.isArray(parsed) && parsed.every((entry) => typeof entry?.cone === "string")) {
      return parsed as ConeTempEntry[];
    }
  } catch (error) {
    console.error("Failed to read stored cone chart", error);
  }
  return DEFAULT_CONE_CHART;
};

export const getConeTemperature = (cone?: string) =>
  cone ? loadConeChart().find((entry) => entry.cone === cone)?.temperatureF : undefined;

export const coneOptions = () => loadConeChart().map((entry) => entry.cone);
