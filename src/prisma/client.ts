...
class PrismaClient {
  ...
  private seed() {
    ...
    this.kilns.push(bigKiln, tinyKiln);

    this.glazes.push(
      { id: uuid(), studioId: studio.id, name: "Fog", brand: "Studio", color: "Gray", coneRange: "5-6", notes: null },
      { id: uuid(), studioId: studio.id, name: "Seaweed", brand: "Studio", color: "Green", coneRange: "5-6", notes: null },
      { id: uuid(), studioId: studio.id, name: "Carbon Trap", brand: "Studio", color: "Black", coneRange: "10", notes: null },
    );

    this.clayBodies.push(
      { id: uuid(), studioId: studio.id, name: "Speckled Stoneware", typicalBisqueTemp: 1828, notes: null },
      { id: uuid(), studioId: studio.id, name: "Porcelain", typicalBisqueTemp: 1828, notes: null },
    );

    const firingStart = new Date();
    const sitterDrop = new Date(firingStart.getTime() + 4 * 60 * 60 * 1000);
    const exampleFiring: Firing = {
      id: uuid(),
      kilnId: bigKiln.id,
      ...
      createdAt: firingStart,
      updatedAt: firingStart,
    };
    this.firings.push(exampleFiring);
    ...
    const project: Project = {
      ...
      clayBodyId: this.clayBodies[0]!.id,
      hasBeenBisque: true,
      bisqueTemp: 1828,
    ...
    const glazeFog = this.glazes.find((g) => g.name === "Fog")!;
    const glazeSeaweed = this.glazes.find((g) => g.name === "Seaweed")!;

    const step1Id = uuid();
    const step2Id = uuid();
    const step3Id = uuid();

    this.projectSteps.push(
      {
        id: step1Id,
